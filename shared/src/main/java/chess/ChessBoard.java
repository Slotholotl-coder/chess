...
public class ChessBoard {
        ...
        for (int i = 0; i < board.length; i++) {
            for (int j = 0; j < board[i].length; j++) {
                if (this.board[i][j] != null) {
                    copy.board[i][j] = new ChessPiece(this.board[i][j].getTeamColor(),
                            this.board[i][j].getPieceType()); // Assuming ChessPiece has a copy
                    // constructor
                }
            }
        }
    ...
    public ChessPiece getPiece(ChessPosition position) {
        if (position.getRow() > 8 || position.getRow() < 1 || position.getColumn() > 8
            ...
            return (board[getCoordinate(position.getRow())][getCoordinate(position.getColumn())] == null
                    ? null :
                    board[getCoordinate(position.getRow())][getCoordinate(position.getColumn())]);
        }
    }
    ...
    public void resetBoard() {
        ...
        for (int i = 1; i <= 8; i++) {
            addPiece(new ChessPosition(2, i), new ChessPiece(ChessGame.TeamColor.WHITE,
                    ChessPiece.PieceType.PAWN));
            addPiece(new ChessPosition(7, i), new ChessPiece(ChessGame.TeamColor.BLACK,
                    ChessPiece.PieceType.PAWN));
        }
