...
public class ChessPiece {
    ...
    private void kingMoves(Collection<ChessMove> moves, ChessBoard chessBoard,
    ...
    private void bishopMoves(Collection<ChessMove> moves, ChessBoard chessBoard,
                             ChessPosition myPosition) {
        for (int i = 1; i <= 8; i++) {
            ChessPosition moveToPosition = new ChessPosition(myPosition.getRow() + i,
                    myPosition.getColumn() + i);
            if (validMove(chessBoard, moveToPosition)) {
                moves.add(new ChessMove(myPosition, moveToPosition, null));
                if (isBlocked(chessBoard, moveToPosition) && canCapture(chessBoard,
                        moveToPosition)) {
                    break;
                }
            } else {
    ...
    private void rookMoves(Collection<ChessMove> moves, ChessBoard chessBoard,
                           ChessPosition myPosition) {
        for (int i = 1; i <= 8; i++) {
            ChessPosition moveToPosition = new ChessPosition(myPosition.getRow() + i,
        ...
        for (int i = 1; i <= 8; i++) {
            ChessPosition moveToPosition = new ChessPosition(myPosition.getRow(),
                    myPosition.getColumn() + i);
            if (validMove(chessBoard, moveToPosition)) {
                moves.add(new ChessMove(myPosition, moveToPosition, null));
                if (isBlocked(chessBoard, moveToPosition) && canCapture(chessBoard,
    ...
    private void knightMoves(Collection<ChessMove> moves, ChessBoard chessBoard,
                             ChessPosition myPosition) {
    ...
    private void pawnMoves(Collection<ChessMove> moves, ChessBoard chessBoard,
                           ChessPosition myPosition) {
        int directionInt = 0;
        switch (pieceColor) {
            case WHITE:
                ...
                if (myPosition.getRow() == 2) {
                    pawnMoveTwo(moves, chessBoard, myPosition, directionInt);
                }
                break;
            case BLACK:
                directionInt = -1;
        ...
        ChessPosition forwardMove = new ChessPosition(myPosition.getRow() + directionInt,
                myPosition.getColumn());
        if (validMove(chessBoard, forwardMove) && !isBlocked(chessBoard, forwardMove)) {
            promotion(moves, myPosition, forwardMove);
        }

        ChessPosition rightDiagonal = new ChessPosition(myPosition.getRow() + directionInt,
                myPosition.getColumn() + 1);
        if (isBlocked(chessBoard, rightDiagonal) && canCapture(chessBoard, rightDiagonal)
                && validMove(chessBoard, rightDiagonal)) {
            promotion(moves, myPosition, rightDiagonal);
        }
        ChessPosition leftDiagonal = new ChessPosition(myPosition.getRow() + directionInt,
                myPosition.getColumn() - 1);
        if (isBlocked(chessBoard, leftDiagonal) && canCapture(chessBoard, leftDiagonal)
                && validMove(chessBoard, leftDiagonal)) {
            promotion(moves, myPosition, leftDiagonal);
        }
    }

    private void pawnMoveTwo(Collection<ChessMove> moves, ChessBoard chessBoard,
                             ChessPosition myPosition, int directionInt) {
        ChessPosition moveTwo = new ChessPosition(myPosition.getRow() + (2 * directionInt),
                myPosition.getColumn());
        if (validMove(chessBoard, moveTwo) && chessBoard.getPiece(new ChessPosition(myPosition.getRow()
                + directionInt, myPosition.getColumn())) == null) {
    ...
    private void promotion(Collection<ChessMove> moves, ChessPosition myPosition,
    ...
    private boolean inBounds(ChessPosition moveToPosition) {
        return moveToPosition.getRow() <= 8 && moveToPosition.getRow() >= 1 && moveToPosition.getColumn()
                <= 8 && moveToPosition.getColumn() >= 1;
    }
