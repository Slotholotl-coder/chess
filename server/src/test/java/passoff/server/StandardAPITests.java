...
public class StandardAPITests {
    ...
    public void successLogin() {
        ...
        Assertions.assertNotNull(loginResult.getAuthToken(), "Response did not return " +
    ...
    public void successRegister() {
        ...
        Assertions.assertNotNull(registerResult.getAuthToken(), "Response did not contain an " +
                "authentication string");
    }
    ...
    public void gamesList() {
        ...
        TestCreateResult game1 = serverFacade.createGame(new TestCreateRequest(game1Name),
                authA.getAuthToken());
        serverFacade.joinPlayer(new TestJoinRequest(ChessGame.TeamColor.BLACK, game1.getGameID())
                , authA.getAuthToken());
        expectedList.add(new TestListEntry(game1.getGameID(), game1Name, null,
                authA.getUsername()));


        //1 as white from B
        String game2Name = "Lonely";
        TestCreateResult game2 = serverFacade.createGame(new TestCreateRequest(game2Name),
                authB.getAuthToken());
        serverFacade.joinPlayer(new TestJoinRequest(ChessGame.TeamColor.WHITE, game2.getGameID())
                , authB.getAuthToken());
        expectedList.add(new TestListEntry(game2.getGameID(), game2Name, authB.getUsername(),
                null));


        //1 of each from C
        String game3Name = "GG";
        TestCreateResult game3 = serverFacade.createGame(new TestCreateRequest(game3Name),
                authC.getAuthToken());
        ...
        serverFacade.joinPlayer(new TestJoinRequest(ChessGame.TeamColor.BLACK, game3.getGameID())
                , authA.getAuthToken());
        expectedList.add(new TestListEntry(game3.getGameID(), game3Name, authC.getUsername(),
                authA.getUsername()));


        //C play self
        String game4Name = "All by myself";
        TestCreateResult game4 = serverFacade.createGame(new TestCreateRequest(game4Name),
                authC.getAuthToken());
        ...
        serverFacade.joinPlayer(new TestJoinRequest(ChessGame.TeamColor.BLACK, game4.getGameID())
                , authC.getAuthToken());
        expectedList.add(new TestListEntry(game4.getGameID(), game4Name, authC.getUsername(),
                authC.getUsername()));
        ...
        Collection<TestListEntry> returnedList =
                new HashSet<>(Arrays.asList(listResult.getGames()));
    ...
    public void uniqueAuthorizationTokens() {
        ...
        Assertions.assertNotNull(loginOne.getAuthToken(), "Login result did not contain an " +
                "authToken");

        TestAuthResult loginTwo = serverFacade.login(existingUser);
        Assertions.assertEquals(HttpURLConnection.HTTP_OK, serverFacade.getStatusCode(),
                "Server response code was not 200 OK");
        Assertions.assertNotNull(loginTwo.getAuthToken(), "Login result did not contain an " +
                "authToken");
